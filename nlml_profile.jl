"""
Profiling of NLML computation to identify specific bottlenecks.
...
x_std, y_std = profile_data_prep_standard()
println("Data size: x=$(length(x_std)), y=$(length(y_std))")

println("Gradient GP data prep:")
x_grad, y_grad = profile_data_prep_gradient()
...
gp_test_std = profile_gp_construction_standard()

println("Gradient GP construction:")
gp_test_grad = profile_gp_construction_gradient()
...
gpx_std = profile_finite_gp_standard()

println("Gradient GP FiniteGP creation:")
gpx_grad = profile_finite_gp_gradient()
...
# @benchmark profile_logpdf_standard()

# @benchmark profile_logpdf_gradient()
...
# In fact, we show that computing C takes the most time. 
# If we have C, then it is very fast. For hyperopt, we need a new C at every test of hyperparams...
...
const k_std = gp_test_std.gp.kernel
const k_grad = gp_test_grad.gp.kernel
