...
@load "grad_bo_himmel.jld2" feval_grad error_grad

running_min = accumulate(min, f.(xs))

Plots.plot(1:length(running_min),norm.(running_min),yaxis=:log, title="Error w.r.t true minimum (2D BO)",
...
Plots.vspan!([n_train,3*n_train]; color=:purple,alpha=0.2, label="")
Plots.plot!(feval_grad,error_grad,label="gradBO")
savefig("2D_error_BO_OUU_himmel.pdf")
